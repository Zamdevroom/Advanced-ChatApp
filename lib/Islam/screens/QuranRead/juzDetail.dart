...
//
//   // Future<Map<Surahs, List<Ayah>>> fetchAyahsGroupedBySurah(int juzNumber) async {
//   //   // Load Quran data
...
//   //   final surahAyahMap = <Surahs, List<Ayah>>{};
...
//
//     // Filter Ayahs by Juz number and group them by Surah
//     final surahAyahMap = <Surah, List<Ayah>>{}; // Corrected type here
//     for (var surah in surahs) {
//       final ayahsInJuz =
//       surah.ayahs.where((ayah) => ayah.juz == juzNumber).toList();
//       if (ayahsInJuz.isNotEmpty) {
...
//                     ),
//                     Expanded(
//                       child: FutureBuilder<Map<Surahs, List<Ayah>>>(
...
// import '../Translation/TranslationControl.dart';
...
class _JuzDetailScreenState extends State<JuzDetailScreen> {
  final settingCont getxSetting = Get.put(settingCont());
  final TranslationControl translationController = Get.put(TranslationControl());

  @override
  void initState() {
    super.initState();
    // translationController.getQuran();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Obx(() {
        ...
        final surahAyahMap = <Surahs, List<Ayahs>>{};
        for (var surah in translationController.quran.value.data!.surahs!) {
          final ayahsInJuz =
          surah.ayahs!.where((ayah) => ayah.juz == widget.juzNumber).toList();
          if (ayahsInJuz.isNotEmpty) {
            surahAyahMap[surah] = ayahsInJuz;
          }
        }
